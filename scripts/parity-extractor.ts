...
function extractBonusData(content: string): CanonicalData['bonuses'] {
  ...
  for (const pattern of bonusPatterns) {
    ...
    if (hexRarity) {
      canonicalRarity = hexRarity.startsWith('0x')
        ? hexToInt(hexRarity)
        : Number.parseInt(hexRarity, 10);
    }
...
function extractPerkData(_content: string): CanonicalData['perks'] {
  ...
  const perkData: Array<{ id: number; name: string; description: string; category: string }> = [
    ...
    },
    {
      id: 8,
      name: 'Grim Deal',
      description: 'I will make you a deal: I will give you experience for your health.',
      category: 'gamble',
    },
    {
      id: 9,
    ...
    },
    {
      id: 18,
      name: 'Anxious Loader',
      description: 'When you can not stand waiting you load faster.',
      category: 'combat',
    },
    {
      id: 19,
    ...
    },
    {
      id: 22,
      name: 'Unstoppable',
      description: 'Monsters can not slow you down with their nasty bites.',
      category: 'special',
    },
    {
      id: 23,
    ...
    },
    {
      ...
      description: 'Empty clip does not prevent you from firing.',
      category: 'combat',
    },
    {
      id: 36,
    ...
    },
    {
      id: 51,
      name: 'Ion Gun Master',
      description: 'You are good with ion weapons. You rock!',
      category: 'special',
    },
    {
      id: 52,
      name: 'Stationary Reloader',
      description: 'It is incredibly hard to reload your weapons.',
      category: 'special',
    },
    {
      id: 53,
    ...
    },
    {
      ...
      description: 'It comes a time in each mans life when he must take a stand.',
      category: 'defense',
    },
    {
      id: 56,
...
function main(): void {
  ...
  const weaponContent = readDecompiledFile(rootPath, '00445_weapon_table_init_004519b0.c');
  const bonusContent = readDecompiledFile(rootPath, '00136_bonus_metadata_init_00412660.c');
  const perkContent = readDecompiledFile(rootPath, '00271_perks_init_database_0042fd90.c');
  const creatureContent = readDecompiledFile(rootPath, '00274_creature_spawn_template_00430af0.c');
  const questContent = readDecompiledFile(rootPath, '00329_quest_database_init_00439230.c');
  ...
  for (const weapon of data.weapons) {
    if (weapon.damage < 1 && weapon.id < 30) {
      warnings.push(
        `Weapon ${weapon.name} (ID ${weapon.id}) has very low damage: ${weapon.damage}`
      );
    }
    if (weapon.damage > 500) {
      warnings.push(
        `Weapon ${weapon.name} (ID ${weapon.id}) has very high damage: ${weapon.damage}`
      );
    }
    if (weapon.clipSize > 200) {
      warnings.push(
        `Weapon ${weapon.name} (ID ${weapon.id}) has very large clipSize: ${weapon.clipSize}`
      );
    }
  }
  ...
  if (verbose) {
    ...
    for (const w of data.weapons.slice(0, 5)) {
      console.log(
        `  ${w.name}: damage=${w.damage.toFixed(2)}, fireRate=${w.fireRate.toFixed(2)}, reloadTime=${w.reloadTime.toFixed(2)}, clipSize=${w.clipSize}`
      );
    }
